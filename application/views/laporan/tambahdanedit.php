...
<form id="form_proses" action="<?= base_url('formproses') ?>" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<input type="date" name="tgl" class="form-control"  required="required" value="<?php echo $tgl; ?>" <?php if ($ket == 'edit') { echo 'disabled="disabled"';} ?>>
	</div>
	<div class="form-group">
		<label for="exampleFormControlTextarea1">Isi Laporan</label>
		<textarea class="form-control" name="isi" rows="3" required="required"></textarea>
	</div>
	<?php if ($ket != 'edit') {
